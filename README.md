...
Update the VITE settings in the new `.env` file. If developing locally, most
settings can stay the same — point `VITE_API_BASE_URL` at your Ephany Framework
backend and you are done.

No API key is required. The app signs in as a user: log in through the UI and it
stores a DRF token. Create an account on the backend with:

```
python manage.py createsuperuser
```

> Do not put an API key in `.env`. Vite inlines `VITE_*` values into the built
> JavaScript bundle, so any key placed there is readable by everyone who loads
> the app. API keys belong in clients that can keep a secret — the Revit plugin,
> CLI scripts, the Smartsheet sync commands.
