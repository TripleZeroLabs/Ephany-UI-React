...
export function SnapshotDetailView() {
  ...
  const tableData = useMemo(() => {
    ...
    return Object.keys(categoryMap)
      .map(cat => ({
        ...
        percentage: totalCategorizedLF > 0 ? Number(((categoryMap[cat] / totalCategorizedLF) * 100).toFixed(1)) : 0,
      }))
  ...
  const displayDonutTotalLF = useMemo(() =>
    donutData.reduce((sum, item) => sum + item.value, 0).toFixed(1),
  [donutData]);
  ...
  const groupedData = useMemo(() => {
    ...
    return Object.values(groups).sort((a, b) => {
      ...
      return String(a.asset[groupSortKey] || "").toLowerCase().localeCompare(String(b.asset[groupSortKey] || "").toLowerCase());
    });
  ...
  if (!snapshot || !project) return <div className="p-12 text-center text-slate-400">Snapshot data missing.</div>;
  ...
  return (
    <div className="mx-auto max-w-7xl py-6 space-y-6">
      ...
      {/* --- REGION 1: ANALYTICS DASHBOARD --- */}
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
        <div className="bg-slate-50/50 px-6 py-4 border-b border-slate-200">
          <h2 className="text-[12px] font-bold uppercase tracking-[0.2em] text-slate-500">Merchandise Analytics</h2>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 divide-y lg:divide-y-0 lg:divide-x divide-slate-200">
          ...
          <div className="lg:col-span-2 p-4 flex flex-col md:flex-row items-center h-[320px] gap-6">
            <div className="flex-1 h-full relative">
              {/* Center Label - Lowered Z-index slightly */}
              ...
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  ...
                  {/* FIX: wrapperStyle ensures tooltip renders ABOVE the absolute center div */}
                  <Tooltip
          ...
          <div className="lg:col-span-1 bg-slate-50/30">
            <div className="px-6 py-3 border-b border-slate-200 bg-slate-50">
              <h3 className="text-[10px] font-bold uppercase tracking-widest text-slate-400">Top 25 Categories</h3>
            </div>
                ...
                <tbody className="divide-y divide-slate-100 bg-transparent">
                  {tableData.map((data, i) => (
                    <tr key={i} className="hover:bg-white transition-colors">
                      <td className="px-6 py-2.5 text-xs font-bold text-slate-700 truncate max-w-[150px]">
                        {data.name}
                      </td>
                  ...
                  <div className="flex items-center justify-between p-4 bg-slate-50/30">
                    <div className="flex items-center gap-4 flex-1">
                      <div className="h-12 w-12 flex-shrink-0 overflow-hidden rounded border border-slate-200 bg-white">
                        {group.asset.catalog_img ? <img src={group.asset.catalog_img} className="h-full w-full object-contain" alt="" /> : <div className="h-full flex items-center justify-center text-[10px] text-slate-300 font-bold uppercase">No Img</div>}
                      </div>
                      <div>
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <button
                        onClick={() => setExpandedAssetIds(p => { const n = new Set(p); n.has(group.asset.id) ? n.delete(group.asset.id) : n.add(group.asset.id); return n; })}
                        ...
                        <div key={item.id} className="flex flex-col sm:flex-row px-6 py-3.5 hover:bg-white transition-colors gap-y-2">
                          ...
                          <div className="flex flex-1 gap-x-12">
                            <div className="flex flex-col">
                                <span className="text-[11px] font-black text-slate-300 uppercase leading-none mb-1">Location</span>
                                <span className="text-[11px] font-bold text-slate-700 leading-none">{item.location || "—"}</span>
                            </div>
                            <div className="flex flex-col">
                                <span className="text-[11px] font-black text-slate-300 uppercase leading-none mb-1">Category</span>
                  ...
                  {sortedInstances.map((inst) => (
                    ...
                    >
                      ...
                      <td className="px-6 py-4">
                        ...
                        <div className="text-[10px] text-slate-400 font-bold uppercase">{inst.asset_details.manufacturer_name}</div>
                      </td>
