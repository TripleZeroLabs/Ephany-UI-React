...
import { fetchVendors, type Vendor } from "../api/vendors.ts";
...
export function VendorsPage() {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    setError(null);
    fetchVendors({ page, pageSize, search: searchTerm.trim() || undefined })
      .then((data) => {
        if (!mounted) return;
  ...
  const columns: ColumnDef<Vendor>[] = [
    { key: "name", header: "Name" },
    {
      ...
      header: "Website",
      render: (v) => v.website
        ? <a href={v.website} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline dark:text-blue-400" onClick={(e) => e.stopPropagation()}>{v.website.replace(/^https?:\/\//, "").replace(/\/$/, "")}</a>
        : <span className="text-slate-400">—</span>,
    },
    { key: "contact_email", header: "Contact Email", render: (v) => v.contact_email || <span className="text-slate-400">—</span> },
    {
      key: "id",
      header: "",
      render: (v) => (
  ...
  if (loading && vendors.length === 0) return <div className="p-6 text-sm text-slate-600 dark:text-slate-300">Loading vendors…</div>;
  ...
  return (
    <div>
      <div className="flex items-center justify-between px-4 pt-4">
        <FiltersPanel searchValue={searchTerm} onSearchChange={(v) => { setPage(1); setSearchTerm(v); }} searchPlaceholder="Search vendors…" />
        <button onClick={() => navigate("/vendors/new")} className="ml-4 shrink-0 px-3 py-2 text-sm font-medium rounded-md bg-indigo-600 hover:bg-indigo-700 text-white transition-colors">+ New Vendor</button>
      </div>
      <PaginationBar />
      <DataTable rows={vendors} columns={columns} getRowKey={(v) => v.id} onRowClick={(v) => navigate(`/vendors/${v.id}/edit`)} />
